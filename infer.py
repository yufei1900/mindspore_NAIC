...
def parse_args():
    ...
    parser.add_argument('--save_path', type=str, default='./val', help='data path')
    ...
    parser.add_argument('--scales', type=float, default = [1], action='append', help='scales of evaluation')
    ...
    parser.add_argument('--ckpt_path', type=str, default='./ckpt/deeplab_v3_s16-170_1484.ckpt', help='model to evaluate')
...
def net_eval():
    ...
    img_lst = os.listdir(args.data_path)
    # network
    ...
    for i, line in enumerate(img_lst):
        img_path = line
        img_path = os.path.join(args.data_path, img_path)
        img_ = cv2.imread(img_path)
        orig_h, orig_w, _ = img_.shape
        ...
        if bi == args.batch_size:
            ...
            for i in range(args.batch_size):
                ...
                label = label.resize((orig_w, orig_h), Image.NEAREST)
                label.save(args.save_path + '/' + img_path.split('/')[-1].split('.')[0] +'.png')
            bi = 0
            batch_img_lst = []
            batch_msk_lst = []
